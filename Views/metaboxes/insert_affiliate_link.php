...
if ( Plugin::get_instance()->has_valid_licence() ) : ?>
    ...
    <div class="form-field form-required">
        ...
        <input name="<?php echo esc_attr( $prefix ) ?>link_id" id="<?php echo esc_attr( $prefix ) ?>link_id" type="number"
               value="<?php echo esc_attr( get_post_meta( $post->ID, $prefix . 'link_id', true ) ) ?>" size="40" required>
        <p>The ID of the affiliate link you wish to insert.</p>
    </div>

    <div class="form-field form-required">
        <label for="<?php echo esc_attr( $prefix ) ?>link_text">Display Text</label>
        <input name="<?php echo esc_attr( $prefix ) ?>link_text" id="<?php echo esc_attr( $prefix ) ?>link_text" type="text"
    ...
    <div class="form-field form-required">
        ...
        <input name="<?php echo esc_attr( $prefix ) ?>link_text_classes" id="<?php echo $prefix ?>link_text_classes" type="text"
               value="<?php echo esc_attr( get_post_meta( $post->ID, $prefix . 'link_text_classes', true ) ) ?>" size="40" required>
    ...
    <div class="form-field form-required">
        ...
        <input name="<?php echo esc_attr( $prefix ) ?>link_banner_classes" id="<?php echo esc_attr( $prefix ) ?>link_banner_classes" type="text"
               value="<?php echo esc_attr( get_post_meta( $post->ID, $prefix . 'link_banner_classes', true ) ) ?>" size="40"
               required>
        <p>Separate ach class with a comma (,) EG class-one, class-two .</p>
    </div>
...
<?php if ( ! Plugin::get_instance()->has_valid_licence() ): ?>
    <p><strong>Affiliate links post sponsor feature only available in PRO version. <a
                    href="<?php echo esc_attr( \Level_Wp\Affiliate_Advantage\Plugin::get_instance()->upgrade_link() ) ?>">Upgrade
